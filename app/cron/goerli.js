...
function getPastEventsScanUrl(_tokenaddress) {
  return process.env.scanurlGoerli + 'api?module=logs&action=getLogs&fromBlock=0&toBlock=latest&address=' + _tokenaddress + '&apikey=' + process.env.scanurlapiGoerlikey;
}
...
async function withdraw({ deposit, currency, amount, recipient, relayerURL, refund = '0', tokenAddress }) { 
  ...
  await privateNado.methods.withdraw(proof, ...args).send({ from: senderAccount.address, value: refund.toString(), gas: 1e6 })
...
async function scanDBexecuteWithdraw({ currency = 'eth'}) {  
  ...
  if (web3 === undefined) {    
    ...
    try {
      web3 = new Web3(new Web3.providers.HttpProvider(process.env.httpproviderGoerli));
    } catch (error) {
      console.log(error)
    }    
  }  
